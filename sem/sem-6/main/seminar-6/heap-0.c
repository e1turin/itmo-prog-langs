...
struct block_id block_allocate(struct heap* heap) {
  ...
  for(size_t i = 0; i < heap_blocks; ++i){
    if(!(heap->is_occupied[i])){    //block_is_free(heap->blocks[i])
      id = i;
      heap->is_occupied[i]=true;
      break;
    }
  }
  if(id == -1){ return block_id_invalid(); }
  return (struct block_id){.valid = true, .value = id, .heap = heap};
}

/* mark block as 'free' */
void block_free(struct block_id b) {
  if(block_id_is_valid && !block_is_free(b)){
