...
/* GNU code style if you wonder, and it's cool, isn't it? */
...
static bool split_if_too_big (struct block_header *block, size_t query)
{
  ...
  const block_capacity query_block_capacity = (block_capacity){query};
  const block_size query_block_size = size_from_capacity (query_block_capacity);

  const block_capacity new_block_capacity = (block_capacity)
	  {block->capacity.bytes - query_block_size.bytes};
  const block_size new_block_size = size_from_capacity (new_block_capacity);

  block_init ((void *)block + query_block_size.bytes, new_block_size, block->next);
  block->next = (void *)((uint8_t *)block + query_block_size.bytes);
  block->capacity = query_block_capacity;
  return true;
}
...
static struct block_header *memalloc (size_t query, struct block_header *const heap_start)
{
  ...
  struct block_header *suitable_block = grow_heap (bsr.block, query);
  if (suitable_block == NULL)
	return NULL;

  bsr = try_memalloc_existing (query, suitable_block);
  if (bsr.type == BSR_FOUND_GOOD_BLOCK)
	{
	  bsr.block->is_free = false;
	  return bsr.block;
...
static struct block_header *block_get_header (void *const contents)
{
